...
function hasFailedStillValidProbeEvidenceForThread(args: {
  ...
  const latestCodexCommentNode = providerCommentNode(args.config, args.thread);
  ...
  const findingBody = providerCommentNode(args.config, args.thread)?.body ?? "";
...
export function buildPostPublicationCodexVerificationTimelineArtifacts(args: {
  ...
  const codexTurnVerificationTimelineArtifacts =
    ...
    codexTurnVerificationHeadSha
      ...
      : null;
  const stillValidRepairProbeThreads = mustFixReviewThreads(args.config, args.reviewThreadsToProcess)
    .filter((thread) =>
  ...
  const stillValidRepairProbeThreadFingerprints =
    processedCodexConnectorReviewThreadFingerprintsForHead(args.config, stillValidRepairProbeThreads, currentPrHeadSha);
