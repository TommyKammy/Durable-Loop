...
test("replay-corpus-promote surfaces advisory promotion-worthiness hints for high-value snapshots", async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "replay-corpus-cli-hints-"));
  const hintedSnapshotPath = path.join(tempDir, "hinted-snapshot.json");
  await fs.writeFile(hintedSnapshotPath, JSON.stringify({
    schemaVersion: 1,
    capturedAt: "2026-03-19T00:00:00Z",
    issue: {
      number: 557,
