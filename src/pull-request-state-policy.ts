...
export function inferStateFromPullRequest(
  ...
  const codexConnectorMustFixThreads = mustFixReviewThreads(config, unresolvedBotThreads);
  ...
  const retryFingerprintForThread = (thread: ReviewThread) =>
    codexConnectorMustFixThreadIds.has(thread.id) ? providerCommentFingerprint(config, thread) : undefined;
