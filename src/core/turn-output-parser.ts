/**
 * Phase 6: Provider-neutral turn output parser.
 *
 * Parses the labeled footer format from agent turn output:
 *   State hint: <state>
 *   Blocked reason: <reason>
 *   Failure signature: <signature>
 *
 * This format is identical across all providers (Codex, OpenCode, Claude Code)
 * because it is produced by the PromptBuilder's footer template, not by the
 * provider CLI. These functions are therefore provider-neutral.
 *
 * Moved from src/codex/codex-output-parser.ts — the only dependency is
 * core/types (RunState, BlockedReason), with no Codex-specific logic.
 */
