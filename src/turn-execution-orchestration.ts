...
import { shouldUseCompactResumePrompt } from "./core/resume-policy";
import {
...
export function selectReviewThreadsForTurn(args: {
  ...
  const codexConnectorMustFixThreadCandidates = mustFixReviewThreads(args.config as SupervisorConfig, activeConfiguredBotThreads);
  ...
  const retryFingerprintForThread = (thread: ReviewThread) =>
    codexConnectorMustFixThreadIds.has(thread.id) ? providerCommentFingerprint(args.config as SupervisorConfig, thread) : undefined;
  ...
  const codexConnectorMustFixThreads = codexConnectorMustFixThreadCandidates.filter((thread) =>
    reviewLoopRetryBudgetAvailable(thread, providerCommentFingerprint(args.config as SupervisorConfig, thread)),
  );
  const codexConnectorReviewChurnDiagnostic = selectChurnAdapter(args.config).buildChurnDiagnostic(
    args.config,
...
export function shouldResumeAgentTurn(args: {
  record: Pick<IssueRunRecord, "executor_session_id" | "state">;
  agentRunnerCapabilities?: Pick<AgentRunnerCapabilities, "supportsResume">;
}): args is {
  record: Pick<IssueRunRecord, "executor_session_id" | "state"> & { executor_session_id: string };
  agentRunnerCapabilities?: Pick<AgentRunnerCapabilities, "supportsResume">;
} {
  const canResume = args.agentRunnerCapabilities?.supportsResume ?? true;
  return Boolean(args.record.executor_session_id) && canResume && shouldUseCompactResumePrompt(args.record.state);
}
  ...
  const turnContext =
    shouldResumeTurn
      ? {
          ...
          sessionId: record.executor_session_id!,
        }
      : {
          ...commonTurnContext,
          kind: "start" as const,
...
export function nextProcessedReviewThreadPatch(args: {
  ...
  const codexMustFixThreadsToTrack = mustFixReviewThreads(args.config as SupervisorConfig, unresolvedConfiguredThreadsToProcess);
            ...
            (state, thread) =>
              nextReviewLoopRetryStateForThread({
                ...
                latestCommentFingerprintOverride: codexMustFixThreadIdsToTrack.has(thread.id)
                  ? providerCommentFingerprint(args.config as SupervisorConfig, thread)
                  : undefined,
              }),
...
export function nextReviewFollowUpPatch(args: {
  ...
  if (mustFixReviewThreads(args.config as SupervisorConfig, postRunConfiguredThreads).length > 0) {
    return defaultPatch;
  }
